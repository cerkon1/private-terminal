...
v0.15 sketch (2026-05-03). M1–M7 shipped, M8 code-complete, M8.5 Maintenance complete, M8.6 Polish complete, S12 release-blocker pass complete, S13 Manage Watchlist refactor complete, **v1.0.0-rc.1 shipped + rc.2 in tester hands**, v1.1 Analysis section Phase 1-3 shipped, v1.2 Pulse killer-feature shipped (S20 design + S20 database reformulation + S21 implementation), Scanner deprecated (S21) + physically deleted (S24), v1.2 UX leverage week shipped (S22 — MACRO-tile retrofit, AVWAP, last_fetch_error, Ctrl+K palette, right-click tile menu, app-wide readability passes), v1.2 responsive density Phase 1 shipped (S23 — ~100-130px reclaim app-wide, tester-confirmed satisfied), v1.2 indicator-surface reorganization shipped (S24 — chip strip merge + multi-anchor AVWAP via chevron popover; Phase 3 modal picker deferred to v1.3 / when library ≥10), MarketHoursStrip dropped as decorative globalism (S24). Phase 4 Analysis tools (COT / AAII / VIX term) deferred from v1.2 (S22 user call).

**v0.15 changes (S23 + S24):**
- **Responsive density Phase 1 (S23).** Tester returned diagnostic info — display 2256×1504 @ 200% Windows scaling = effective 1128×752 CSS px. 12 edits across 8 files reclaimed ~100-130px app-wide. Chart: legend `right: 360` no-fly zone, dataZoom slider h:26 cyan handles, ATR pane indentation fix (hide boundary labels + bounded-decimal formatter), subpane corner labels (`VOL` / `DD` / `RSI` / `ATR` top-left badges via `cornerLabelGraphic` helper). Chrome trim: AppHeader 2→1 row, StatusBar tightened in place, FeatureChart liability footer removed (principle 9 satisfied via Features-tab card), MarketHoursStrip dropped time field, Pulse banner tagline + TabIntro subtitle removed. Analysis tab labels shortened (`Recession Prob → Recession`, `Financial Conditions → FCI`, `Regime Quadrant → Regime`) so 7-tab strip fits one row at constrained widths. `analysis_tools.display_name` migration via idempotent UPDATE statements at boot — no schema change. Phase 2 (`@media (max-height)`) and Phase 3 (Settings density toggle) **explicitly deferred indefinitely** — tester confirmed satisfaction with rc.2; project closed.
- **Scanner physically deleted (S24).** S21's "preserve one release for revert safety" target satisfied by rc.2. Removed: `Scanner.tsx`, `ScannerRow` struct + `scanner_snapshot` IPC, lib.rs handler, Scanner-only CSS. Preserved: `prime_scanner_histories` IPC (still used by Pulse banner), `.state-badge` base CSS (used by Settings → Appearance palette preview), idempotent `UPDATE … user_hidden=1 WHERE id='scanner'` in seed.sql (defensive on old DBs). FeaturesTab Scanner card replaced with Pulse card.
- **MarketHoursStrip dropped (S24).** Eight-exchange row at top of every screen ("NYSE TSX LSE TYO HKG SSE ASX KRX") was decorative globalism — 5 of 8 exchanges have no watchlist tickers, direction-since-close was already shown in INDICES tiles below with full numbers + currency, open/closed is one binary signal. Removed end-to-end: `MarketHoursStrip.tsx`, `App.tsx` mount, `EXCHANGE_INDEX_MAPPING` + `MarketIndexQuote` + `list_market_index_quotes` IPC in `ticker_cmds.rs`, lib.rs handler, `.market-hours-strip` + `.market-chip*` CSS, FeaturesTab card. Reclaim ~34-40px vertical chrome app-wide. Feature #2 marked DROPPED in Features table (joins #5 / #8 / #10 in dropped column).
- **Chip strip merge — Phase 1 of indicator-surface reorganization (S24).** VRVP / DD / AVWAP moved out of FeatureChart toolbar into IndicatorPanel as second chip group with thin vertical separator. State (`showVrvp` / `showDrawdown` / `showAvwap`) lifted from FeatureChart to TickerDashboard (same KV keys, no migration). Toolbar shrinks to `AUTO Y · VOL · PNG`. Native `title=` hover tips per chip. New `OverlayChips.tsx` (~50 LOC).
- **Multi-anchor AVWAP — Phase 2 of indicator-surface reorganization (S24).** Anchors persisted per-(ticker, dataSource) via single global dict at `session.feature_chart_avwap_anchors` (avoids dynamic-key load race in `usePersistedState` — LESSONS FE-9). Soft cap 5; same accent-amber for all anchors with ECharts series-naming for tooltip identification; toggle-off persists; idempotent click; **Clear all disables AVWAP**. Anchor management via `▾` chevron button on AVWAP chip → `AvwapAnchorsPopover` (~110 LOC) anchored beneath chip via CSS only (`.indicator-chip-wrap` position:relative + child position:absolute). Generalizes for M10 — when per-indicator parameter popover lands, it slots into the same mechanism.
- **Chip + gear visual refinement (S24).** Chips converted from `<label>` + `<input type="checkbox">` to `<button aria-pressed>`; checkbox encoded same state as chip color, was redundant. Gear adopts chip's on-state cyan styling (border + color + tinted bg) at resting opacity 0.55, hover opacity 1.0 — pure brightness shift, no color jumps. `:has()` corner-squaring so chip + gear read as one continuous element.
- **Single-line ticker + title (S24).** `.feature-chart-pane__title-block` flipped from flex-column to flex-row baseline. ~22-26px reclaim per opened chart. Defensive ellipsis fallback for over-long names.
- **Indicator-surface Phase 3 (modal picker + active-only chip strip) deferred to v1.3 / when indicator library hits ~10 active categories.** Phase 1 + Phase 2 land the user-visible win in one session; picker is engineering for hypothetical 15+ indicator scale we don't have today (6 indicators).
