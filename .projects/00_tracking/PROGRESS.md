...
## Current Focus
**S24 (2026-05-03) — UX honesty pass + indicator-surface reorganization. 8 commits on master.** Same-day continuation of S23 (tester confirmed Phase 1 chrome-trim work, freeing the session). Theme: every commit removed something that didn't earn its chrome cost OR shipped something that should have always been there. (1) **Scope guards** (3 commits): Scanner physically deleted (rc.2 was the S21 "preserve one release" target — `Scanner.tsx` + `scanner_snapshot` IPC + Scanner-only CSS removed; `prime_scanner_histories` retained for Pulse banner); Project 04 closed (tester satisfied, Phase 2/3 deferred indefinitely); MarketHoursStrip dropped end-to-end as decorative globalism — eight-exchange row carried zero value when 5/8 exchanges have no watchlist tickers and direction-since-close was already in INDICES tiles below; reclaimed ~34-40px vertical chrome app-wide. DESIGN.md feature #2 marked DROPPED, joining #5/#8/#10 in the dropped column. (2) **Indicator-surface reorganization** (4 commits): user-driven design discussion landed on the chip-strip-over-right-rail layout (Option 1, chart-first) after surveying TradingView / ThinkOrSwim / Bloomberg patterns. Phased path picked: Phase 1 (merge VRVP / DD / AVWAP into IndicatorPanel as second chip group, toolbar shrinks to AUTO Y · VOL · PNG, hover tips via native `title=`) + Phase 2 (multi-anchor AVWAP via ⚙ chevron popover on chip — single global dict at `session.feature_chart_avwap_anchors` keyed by `<ticker>:<dataSource>`, soft cap 5, same accent-amber for all anchors, toggle-off persists, idempotent click, Clear all disables AVWAP) both shipped in-session; Phase 3 (modal picker + active-only display) **deferred to v1.3 / when indicator library hits ~10**. AVWAP polish (Clear all → setShowAvwap(false); ⚙ → ▾ glyph swap for cleaner Windows font rendering); chip + gear visual refinement (chips → button+aria-pressed dropping redundant inline checkboxes; gear adopts chip's on-state cyan with opacity-only hover for visual continuity; `:has()` corner-squaring so chip + gear read as one element). (3) **Bonus** (1 commit): `^GSPC` / `S&P 500` stacked vertically in feature-chart header → flipped `flex-direction: column → row` with baseline alignment and ellipsis fallback; ~22-26px reclaim per chart. **All gates clean every commit** (cargo / 29 tests / tsc / vite). **rc.2 in tester hands; NSIS rebuild deferred** until next session — user signaled wanting strategic "overall purpose" conversation before more code. **Next session entry point: strategic conversation (not implementation), then NSIS rebuild + ship to tester.** See `memory/s24_decisions.md`.
...
### S24 — UX honesty pass + indicator-surface reorganization (2026-05-03)

Same-day continuation of S23. Tester confirmed satisfaction with the rc.2 build (carrying S23 chrome-trim work), freeing this session to attack the next wave of chrome-pretense issues the user surfaced as we worked. Theme: every commit either removed something that didn't earn its chrome cost OR shipped something that should have always been there. 8 commits on master. Decisions captured in `memory/s24_decisions.md`.

**Master timeline.**

| Commit | Topic |
|---|---|
| `33f1bfd` | chore: drop deprecated Scanner — Pulse subsumed it (S21 deferred cleanup) |
| `f725b54` | docs: close project 04 — tester satisfied with Phase 1 chrome trim |
| `99ca01f` | chore: drop MarketHoursStrip — decorative globalism, zero user value |
| `1ae65cf` | feat(charts): merge VRVP / DD / AVWAP into chip strip alongside indicators |
| `a9f9a40` | feat(charts): multi-anchor AVWAP via gear popover on the chip |
| `afb6040` | fix(charts): AVWAP polish — Clear all disables the indicator; chevron over gear glyph |
| `833a667` | fix(charts): drop redundant chip checkboxes; integrate gear with chip on-state |
| `405799b` | fix(charts): single-line ticker + name in feature-chart header |

**(1) Scanner physically deleted (`33f1bfd`).** S21 soft-deleted Scanner via `user_hidden=1` and unrouted it from sidebar; preserved code one release for revert safety. **rc.2 is that release.** Files removed: `Scanner.tsx` (~248 LOC), `ScannerRow` struct + `scanner_snapshot` IPC (~125 LOC) from `indicator_cmds.rs`, handler from `lib.rs::generate_handler!`, Scanner-only CSS (`.scanner-prime-status`, `.scanner-table*`, `.state-badge--*` variants, `.state-filter*`). Preserved deliberately: `prime_scanner_histories` IPC (still used by Pulse banner — IPC name retained for backwards compat), `.state-badge` base CSS (used by Settings → Appearance palette preview), idempotent `UPDATE … user_hidden=1 WHERE id='scanner'` in seed.sql (defensive on old DBs). Replaced FeaturesTab "Scanner" about-card with "Pulse" about-card — Pulse had zero Features-tab coverage before, gap closes. 7 files, +26 / −513 lines.

**(2) Project 04 closed (`f725b54`).** Tester re-tested rc.2 build (S23 chrome-trim work) and confirmed satisfaction with the viewing-element changes. Phase 2 (`@media (max-height)` density rules) and Phase 3 (Settings → Density toggle) deferred indefinitely — Phase 1 reclaim sufficed. `.projects/04_responsive_density/design.md` status header set to **CLOSED**; MEMORY.md memory-index entry updated. No code change.

**(3) MarketHoursStrip dropped (`99ca01f`).** User flagged the eight-exchange row at the top of every screen as juvenile pretense ("acting like it's grown up"). Honest accounting: 5 of 8 exchanges (LSE/TYO/HKG/SSE/KRX) carried zero watchlist tickers; direction-since-close was already shown in INDICES tiles three rows below with full numbers + currency; open/closed is one binary signal that doesn't justify 8 cells. Only honest argument was branding; rejected because Bloomberg has 8 exchanges because Bloomberg's customers actually trade them. Removed: `MarketHoursStrip.tsx` (deleted), `App.tsx` strip mount, `EXCHANGE_INDEX_MAPPING` const + `MarketIndexQuote` struct + `list_market_index_quotes` IPC in `ticker_cmds.rs`, lib.rs handler, `.market-hours-strip` + `.market-chip*` CSS (~92 lines), Features-tab card. **DESIGN.md feature #2 marked DROPPED** — joins #5 (Watchlist Performance, S10), #8 (Keyboard shortcuts, S9), #10 (Scanner, S21) in the dropped column. Reclaim: **~34-40px vertical chrome app-wide** — bigger single win than any S23 reclaim. Pattern: any feature that can't justify its chrome cost gets killed.

**(4) Phase 1 chip strip merge (`1ae65cf`).** User-driven design discussion landed on the diagnosis that the FeatureChart had two competing rows of "indicators" — IndicatorPanel per-ticker chips above the chart vs the `AUTO Y · VOL · VRVP · DD · AVWAP · PNG` toolbar inside the chart. Surveyed industry patterns (TradingView's chip strip + modal picker; ThinkOrSwim's right rail; Bloomberg's command-driven; ECharts demos pill-row), ASCII-mocked two endgame layouts (chart-first chip strip + popover sub-state vs workshop-first right rail). **Picked Option 1** — chart real estate is the load-bearing constraint at tester's 1128px effective width; right rail's main advantage (persistent sub-state visibility) duplicates what's already drawn on the chart (anchor markers, fib levels, alert lines render visually). **Phased path** picked over full path: Phase 1 (merge) + Phase 2 (multi-anchor AVWAP) ship now; Phase 3 (modal picker + active-only display) deferred to v1.3 or when indicator library hits ~10 active categories. Implementation: lifted `showVrvp` / `showDrawdown` / `showAvwap` `usePersistedState` calls from `FeatureChart` up to `TickerDashboard` (same KV keys, no migration); new `OverlayChips.tsx` (~50 LOC) renders the three frontend-overlay chips with same `.indicator-chip` styling for visual parity; both groups (Rust-backed per-ticker indicators + frontend app-wide overlays) live inside a new `.chip-strip` wrapper with thin vertical separator; native `title=` hover tips per chip with descriptions lifted from FeaturesTab card copy; toolbar shrinks to `AUTO Y · VOL · PNG`; behavior parity (every toggle works identically).

**(5) Phase 2 multi-anchor AVWAP (`a9f9a40`).** S22's single-anchor AVWAP was *transient by design* ("exploratory"). Multi-anchor flips the use case: 2-3 anchors at meaningful events (earnings / ATH / news pivots) are tracked positions worth keeping across sessions. **Six locked decisions:** (a) per-ticker persisted via single global dict at `session.feature_chart_avwap_anchors`, keyed by `<ticker>:<dataSource>`, sorted ISO date arrays — single load on mount, avoids the dynamic-key load race in `usePersistedState` (LESSONS FE-9); (b) soft cap of 5 anchors, clicks past cap silently ignored, popover shows `(N/5)`; (c) same accent-amber color for all anchors, identification via popover anchor list + ECharts tooltip names each line series `AVWAP @ <date>`; (d) toggle-off persists anchors (toggle becomes show/hide layer); (e) click-on-existing-anchor idempotent (explicit `×` is the canonical remove path); (f) Clear all disables the indicator (exit ramp — flips `setShowAvwap(false)` so the empty-state hint doesn't dangle). New `AvwapAnchorsPopover.tsx` (~110 LOC) anchored beneath AVWAP chip via CSS only (parent `.indicator-chip-wrap` is `position: relative`; popover is `position: absolute, top: 100%, right: 0`) — no measurement, no ref dance. Closes on click-outside via `mousedown` capture (TileContextMenu pattern, S22 LESSONS FE-7) or Escape. **Generalizes for M10:** when per-indicator parameter popover lands (post-v1), it slots into the same `.indicator-chip-wrap` mechanism. AVWAP series builder iterates anchors → one line series per anchor (named for tooltip identification) + one combined scatter for all anchor markers (single z-stack, single render). FeatureChart's `avwapAnchor: string | null` local state + reset effects removed (anchors are now external); accepts `avwapAnchors: string[]` prop and `onAvwapAnchorClick` callback; ZRender click handler invokes the callback (cap + idempotency in TickerDashboard).

**(6) AVWAP polish (`afb6040`).** User feedback during smoke test: (a) Clear all left the indicator toggled on with empty-state hint dangling at bottom of chart — fixed: Clear all also flips `setShowAvwap(false)`. Existing useEffect closes popover automatically. (b) ⚙ unicode gear glyph rendered chunky on Windows fonts — swapped to `▾` (small chevron). Reads as "open more details", matches dropdown caret convention, renders cleanly across OS fonts at 11px.

**(7) Chip + gear visual refinement (`833a667`).** User flagged checkbox redundancy and gear hover as flakey. Diagnosed two stacked issues: (a) checkboxes inside chips encoded the same state as the chip's cyan-on/dim-off color signal — converted `<label>` + `<input type="checkbox">` to `<button aria-pressed>` (canonical toggle-button a11y pattern); same hit area, same toggle semantics, no double-encoding; CSS dropped `.indicator-chip input[type='checkbox']` rule, added `background: transparent` to `.indicator-chip` (button has UA bg that needs explicit override), added `.indicator-chip--on:hover` so on-state hover keeps cyan instead of falling back to text-primary gray. (b) Gear button rebuilt for visual continuity — previously gray border + tertiary text at rest, hover to cyan border + cyan text (visible seam between chip and gear at rest; two color jumps on hover). Now gear ALWAYS uses cyan border + cyan text + tinted bg matching chip on-state (gear only ever shown when chip is on), resting opacity 0.55, hover opacity 1.0 — pure brightness shift, no color transitions. `.indicator-chip-wrap:has(.indicator-chip__gear) > .indicator-chip { border-top-right-radius: 0; border-bottom-right-radius: 0 }` squares off the chip's right corners when gear is attached so the pair has no notch. **Pattern: opacity over color for hover transitions on elements adjacent to colored siblings.**

**(8) Single-line ticker + title (`405799b`).** User flagged `^GSPC` stacked over `S&P 500` even with horizontal room. Root cause: `.feature-chart-pane__title-block { flex-direction: column }`. Flipped to `row + align-items: baseline + gap: var(--space-sm)`. Defensive: `min-width: 0` so title-block flex-shrinks past intrinsic content width when chip strip is wide; `white-space: nowrap + text-overflow: ellipsis` on h2 so over-long names truncate rather than wrap or push the chip strip off-row at constrained widths. Reclaim: ~22-26px per opened chart.

**Course corrections in-session.**
- **Right-rail vs chip-strip survey before mocks.** User asked for industry-pattern survey before deciding; surveyed TV / TOS / Bloomberg / ECharts demos. Right rail rejected because its main advantage (persistent sub-state visibility) duplicates what's already drawn on the chart.
- **Phased path over full path.** Full path ships nothing for 4-5 sessions; phased lands the user-visible win in one. Picker (Phase 3) is engineering for hypothetical 15+ indicator scale we don't have.
- **MarketHoursStrip — agreed on first proposal** because the steel-man fell apart immediately. Eight-exchange decoration on a three-exchange app is dishonest UX.
- **Clear all behavior bug** caught by user in smoke test. Fixed in follow-up commit.
- **Gear hover flakiness + checkbox redundancy** diagnosed as two stacked issues, fixed in same commit.
- **Ticker + title stacking** — vertical chrome elimination continued from S23.

**Build artifacts.**
- 8 commits on master, working tree clean at session-end.
- Gates clean every commit: `cargo check`, `cargo build` no warnings, `cargo test --lib` 29/29 pass (9 cross_section + 20 analysis math), `tsc --noEmit` EXIT=0, `npm run build` consistently 3.8–5.4s.
- New files: `OverlayChips.tsx`, `AvwapAnchorsPopover.tsx`, `memory/s24_decisions.md`. Deleted files: `Scanner.tsx`, `MarketHoursStrip.tsx`. Net code lines: +431 frontend / −513 dead code (Scanner+MarketHoursStrip deletions exceeded new feature lines).
- Bundle: 1462.73 → 1463.38 kB (+0.7 kB; popover + chip strip are small). CSS: 48.51 → 49.84 kB (+1.3 kB; popover styles).
- Two screenshots committed mid-session: `Screenshot 2026-05-04 075636.png` (drove the MarketHoursStrip removal call).
- No Rust changes after MarketHoursStrip removal — Phase 1 + Phase 2 are pure frontend.

**What didn't ship (for honesty).**
- **Phase 3 modal picker** — deferred to v1.3 / post-10-indicators. No code touched.
- **M10 per-indicator parameter popovers** — only AVWAP got a popover; SMMA / RSI / ATR / VRVP / DD still use defaults. M10 is post-v1 anyway. The popover *mechanism* (`.indicator-chip-wrap` + position:absolute) is in place for them to slot into when M10 lands.
- **NSIS rebuild for tester** — recommended at session-end; deferred until next session per user signal.
- **Strategic "overall purpose" conversation** — flagged by user as the next agenda item; not started.

**Next session entry point.**
- **Strategic conversation about overall purpose for the app** — user explicitly signaled this as the next agenda. Not implementation. Topics likely: indicator-library extensibility roadmap, target audience (personal-only vs friends-and-family vs giveaway), where the moat is, what features earn their place. After alignment lands, decide what (if anything) goes into v1.2 next.
- **Then NSIS rebuild + ship to tester** — same `npm run tauri:build` workflow per S22 setup. Carries S23 + S24 changes.
- **v1.2 priority queue post-S24:** thinned. **Shipped this session and crossed off:** Phase 1 chip strip merge, Phase 2 multi-anchor AVWAP, MarketHoursStrip removal, Scanner physical deletion, Project 04 closure, single-line ticker + title. **Remaining:** Vol Cone subpane (~1 evening), Return Distribution (~1 evening), Seasonality heatmap (~1 evening), CoinGecko fetcher (weekend, backend-heavy), bull/bear VRVP split, true log Y-axis (Path a — manual `log10()`), multi-ticker overlay chart (M9 carry-over). Phase 3 (modal picker + active-only chip strip) explicitly deferred to v1.3 or once indicator library hits 10+. Phase 4 Analysis tools (COT / AAII / VIX term) still deferred per S22 user call.
- **rc.2 in tester hands** — no code commitments on rc.2 unless tester surfaces issues.
