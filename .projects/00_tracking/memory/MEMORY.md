...
## Project Status
- **v1.1 Analysis section Phase 1 shipped on master (2026-04-29, S16).** Correlations + Yield Curve tabs live behind ANALYSIS pinned sidebar entry. Backend: `src-tauri/src/analysis/` module (no trait, const registry + per-tool typed compute free functions, alignment helper with 50%-coverage gate, 5 IPC commands, 10 math unit tests). Frontend: 4 React components (`TickerChipPicker`, `CorrelationsTab`, `YieldCurveTab`, `AnalysisLayout`), `useRecessionBars` hook with module-level cache, ECharts dual-pane yield-curve rendering with NBER recession-bar overlays. New schema: `analysis_tools` table + `tile_visible` column on `fred_series`. Seed: USREC + DGS3MO/DGS2/DGS5/DGS30 (analysis-only, hidden from MACRO) + 2 analysis_tools rows + new 'analysis' sector_groups row. Smoke-tested live: BTC×ETH=0.84, recession bars over 1990/2001/2008-09/2020, 11,162 spread observations.
- **Repo infrastructure shipped same session (S16):** git init at v1.0.0-rc.1, `.projects/<NN_name>/` reorganization with credential-isolated `infrastructure/` folder, archive convention for PROGRESS pruning (`archive/README.md` + first archive `2026-04-29_v1_0_build_arc.md` carrying S1–S13). Master at `ef12190` after fast-forward merge of `feature/v1.1-analysis-phase-1`.
- **Awaiting cold-eye tester feedback on v1.0.0-rc.1** (parallel track to v1.1 Phase 1). No code commitments on RC1 until feedback lands.
...
- [v1.1 Analysis Phase 1 design](../../02_v1_1_analysis/v11_analysis_design.md) — full spec for cross-asset Analysis section (registry-driven tabs, 4-phase plan). Phase 1 (Correlations + Yield Curve) shipped S16; S15 implementation plan executed verbatim. Phase 2 (Pairs + RRG, zero new data) is the natural next chunk.
...
1. API-key management UI vs env-var-first → current state: env-var-first via `dotenvy`; proper UI deferred to M8 settings modal
2. Command palette fuzzy matcher — Fuse.js vs simple custom → M9
3. Yahoo TSX after-hours cadence → blocked on adaptive-TTL infrastructure (see `memory/m3_ticker_decisions.md`)
