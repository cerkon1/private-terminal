...
## Project Status
- **S21 (2026-05-02) — Pulse implementation + Scanner deprecation, uncommitted at session-end.** v1.2 Pulse killer feature shipped end-to-end: `cross_section/` Rust module (mod / compute / percentile / tests + `commands/cross_section_cmds.rs`) computing REGIME / AGE / LEVEL / RSI / ATR / VOL / DD across every leaf sector_group ticker + every FRED series, percentile baselines on trailing-5y windows, universal indicator compute (bypasses per-ticker `indicator_settings`); 9/9 cross_section + 20/20 analysis math tests green. Frontend `PulseDashboard.tsx` (~430 LOC) ships Style C saturated-block cells + Palette 1 R→Y→G + neutral-hold saturation curve + heavy section header + top banner with PULSE branding + `<TabIntro>` per S17 pattern. Ticker-column click → localStorage handoff → TickerDashboard auto-opens feature chart on land (always-clear-on-mount cleanup). Scanner subsumed: PRIME moved into Pulse banner contextually (only renders when `noBarsCount > 0`), RECOMPUTE killed as dead weight, scanner soft-deleted via `user_hidden=1`. PRIME failures surfaced inline below banner with per-ticker error + exchange-suffix hint. Light theme considered + deferred. 7 modified + 7 new files; awaiting commit. See `memory/s21_pulse_implementation_decisions.md`.
